...
app.post("/register", (request, response) => {
    ...
    db.query(searchComand, [user.email], (error, data) => {
        ...
        if(data.length !== 0){
            response.json({message: "Já existe um usuário com esse e-mail.", userExists: true})
            return
        }
...
app.post("/login", (request, response) => {
    ...
    db.query(searchComand, [user.email], (error, data) => {
        ...
        if(data.length === 0){
            response.json({message: "Não existe nenhum usuario cadastrado com esse e-mail!"})
            return
        }
        ...
        response.json({message: "Credenciais invalidás! Tente novamente"})
    })
})
...
app.get("/getname", (request, response) => {
    ...
    response.json({ name: decoded.name})//ver video 22 para resolver o back-end
})  
...
app.get("logout", (request, response) => {
    ...
    response.json({ email: decoded.email})
})

app.get("/rankin", (request, response) => {
    const token = request.headers.authorization
